...
const todos = JSON.parse(localStorage.getItem("todos")) || []; // Initialize todos array from local storage or as empty array
...
function deleteTodo(btn) {
  ...
  saveTodosToLocalStorage(); // Remove item from array
  Displaytodos(); // Update the displayed list
}
...
function editTodo(btn) {
  ...
  editForme.addEventListener("submit", (e) => {
    ...
    saveTodosToLocalStorage(); // Save updated todos to edition
    Displaytodos(); // Update the displayed list
  });
